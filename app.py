...
if st.session_state.page == "welcome":
    ...
    st.markdown(
        ...
        border-radius:12px; box-shadow:2px 2px 8px rgba(0,0,0,0.2);">
            <h1 style='font-size:38px; font-weight:bold;'>Interactive Online Sizing Framework for Grid-Connected Photovoltaic Systems</h1>
            <p style='font-size:20px;'>Hello! This tool will assist you in designing and sizing your PV modules.</p>
        </div>
        """,
        unsafe_allow_html=True
    )
...
# =====================================================
# PAGE 2: PART A - DIMENSIONING (TIDAK DIUBAH)
# =====================================================
    ...
    st.markdown(
        """
        <div style="padding:15px; border-radius:10px; background-color:#f7f9fc; border-left:6px solid #4A90E2;">
    ...
    st.success(f"Orientation: {orientation} | Modules Upwards: {N_up} | Modules Across: {N_across} | Total Modules: {N_max}")
    ...
    st.info(f"Recommended Orientation: {best_orientation} | Maximum PV Modules: {best_count}")
...
# =====================================================
    ...
    # STEP 2: Required Inverter Power
    P_inv_required = (rated_power * best_count) / fi
    ...
    st.success(f"Final Ns_max: {Ns_max} | Final Ns_min: {Ns_min} | Range: {Ns_min}-{Ns_max}")
    ...
    st.metric("Recommended Modules in Series (Ns_rec)", f"{Ns_rec}")

    # STEP 6: Np_max per MPPT
    ...
    st.metric("Maximum Strings per MPPT", f"{Np_max_per_MPPT}")

    # STEP 7: Strings per MPPT
    strings_per_MPPT = math.ceil(best_count / Ns_rec)
    st.metric("Result Configuration (Strings per MPPT)", f"{strings_per_MPPT}")
